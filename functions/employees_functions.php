...
/*
 * function list_users(){ include 'conf/config.php'; include 'conf/opendb.php'; echo '<table class="employee_home_table"> <thead valign="top"> <th>Full Name</th> <th>User Name</th> <th>Permissions</th> </thead> <tbody valign="top">'; $result=mysqli_query("SELECT * FROM employees WHERE user='1' AND cancel_status='0' ORDER BY id DESC" , $conn); while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) { echo ' <tr> <td>'.$row[full_name].'</td> <td>'.strtolower($row[employee_name]).'</td> <td align="center"><a href="employees.php?job=access&id='.$row[id].'" ><img src="images/lock.png" alt="Edit" width="24" height="24"/></a></td> </tr>'; } echo '</tbody></table>'; include 'conf/closedb.php'; }
 */
...
function list_user_module($user_id) {
	...
	$result = mysqli_query ( "SELECT * FROM user_has_module WHERE user_id='$user_id'", $conn );
